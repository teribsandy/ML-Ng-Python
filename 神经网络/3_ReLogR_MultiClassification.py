...
def one_vs_all(X,y,l,K):
    ...
    for i in range(1,K+1):
        ...
        res=minimize(fun=regularized_cost,x0=theta,args=(X,y_i,l),method='TNC',jac=regularized_gradient)
        alltheta[i-1]=res.x
    return alltheta

def predict_all(X,alltheta):
    allh=sigmoid(X@alltheta.T) #(5000,10)
    h_argmax=np.argmax(allh,axis=1) #(5000,)
    h_argmax=h_argmax+1
    return h_argmax
